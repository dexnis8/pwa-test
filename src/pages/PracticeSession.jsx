...
const PracticeSession = () => {
  ...
  // Utility function to format explanations (markdown-ish text + real LaTeX rendering)
  const formatExplanation = (explanation) => {
    ...
    formatted = formatted.replace(/\bimes\b/g, "Ã—");
  ...
  // Lighter-weight formatter for question stems and answer options:
  // fixes encoding issues, renders real LaTeX, and supports basic bold/italic,
  // without the paragraph/bullet-list wrapping used for longer explanation text.
  ...
  const handleImageGenerated = useCallback((imageUrl) => {
  ...
  const shareToTwitter = () => {
    ...
    const message = `I scored ${score}/${questions.length} in ${subject} practice on Pace App! Join my journey to exam success: https://app.paceapp.ng/`;
  ...
  // For comprehension-style questions, find the sibling questions that
  // share the same passage so we can show "Question X of Y for this passage"
  const passageSiblings = (currentQuestionForContext) =>
    currentQuestionForContext?.passageGroupId
      ? questions.filter(
          (q) => q.passageGroupId === currentQuestionForContext.passageGroupId,
        )
      : null;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="px-5 pt-5">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
            <div className="flex justify-between items-start mb-2">
              ...
              {(() => {
                const siblings = passageSiblings(currentQuestion);
                if (!siblings) return null;
                const position =
                  siblings.findIndex((q) => q._id === currentQuestion._id) + 1;
                return (
                  <span className="text-xs text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full whitespace-nowrap ml-2">
                    Passage question {position} of {siblings.length}
                  </span>
                );
              })()}
            </div>
            ...
            <div
              className="text-sm text-gray-800 leading-relaxed katex-content"
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(
                  formatQuestionContent(currentQuestion.passage),
      ...
      <div className="p-5">
        <div className="bg-[#1D7A8C] text-white p-5 rounded-xl mb-4">
          <div className="text-center mb-4">
            {currentQuestion.instruction && (
              <p className="text-xs text-center my-2 mx-2 text-white italic mb-2">
            ...
            <div
              className="text-left font-medium katex-content"
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(
          ...
          {currentQuestion.options.map((option) => (
            ...
            >
              ...
              <span
                ...
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    formatQuestionContent(option.text),
                    KATEX_SANITIZE_CONFIG,
                  ),
                }}
              />
